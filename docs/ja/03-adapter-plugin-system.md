...
| エッジ | すべてのグラフの全エッジを追加 |
| 重複除去 | エッジの重複除去はなし — 同じsrc/dst/relationが複数回現れる可能性あり |
| **ID正規化** ✨ | `spec::1.1` (SpectraAdapter) と `docs.auth.1.1` (HeuristicAdapter) を**自動的に同一specと認識**し、単一の正規ノードに統合。エッジはリダイレクトされ、`spec::` プレフィックスノードは削除、エイリアスは正規ノードの `metadata.aliases` に記録される |

**ID正規化のマッチング戦略:**

1. **完全サフィックス**: `spec::1.1` は `.1.1` で終わるheuristic IDとマッチ
2. **部分サフィックス**: `spec::3.1.2` は `docs.auth.3.1.2` とマッチ
3. **タイトルフォールバック**: 数値サフィックスがマッチしない場合、タイトルを比較
4. **非マッチ時**: `spec::` ノードはそのまま保持
