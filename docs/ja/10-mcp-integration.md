...
指定された仕様を実装するコードを検索します。**あいまいSpec解決**に対応しており、IDの後方一致やタイトルでの検索が可能です。

- **必須パラメータ**: `spec_id` — 例：`"1.1"`, `"TraceNode"`, `"build_heuristic_graph"`
- **解決順序**: 完全ID → `spec::` 接頭辞 → ID後方一致 → タイトル部分一致 → 見出しテキスト
- **複数マッチ**: 複数のspecがマッチした場合、すべての結果と実装アーティファクトを返します
- **戻り値**: 実装コード/テストファイルのリスト（信頼度、エビデンス、関数レベルエッジ含む）
- **ユースケース**: Agent が「spec TraceNode を実装しているのは？」と質問
